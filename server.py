...
#     # Startup
#     heartbeat_thread = asyncio.create_task(receive_vehicle_position())
#     yield
#     if heartbeat_thread:
...
# async def receive_vehicle_position():  # Actively runs and receives live vehicle data on a separate thread
#     '''
...
#             print(f"Client disconnected ({len(self.active_connections)} active)")
...
#     print("CONECTING to flight controler")
#     await manager.connect(websocket)
#     try:
#         while True:
#             data = await websocket.receive_text()  # wait for client message
#             # result = await asyncio.to_thread(receive_vehicle_position)
